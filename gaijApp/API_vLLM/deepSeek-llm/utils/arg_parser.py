...
def argparser_promptType():
    ...
    parser.add_argument('--info_Flag',type=lambda x: x.lower() in ['true', '1', 'yes'], required=False, help='Flag to ask for the prompt to be in Norwegian - default FALSE')
...
def argparser_prompt_info():
    " parser with the info that will be extracted if the info type of prompt is selected"
    parser = argparse.ArgumentParser(description='prompt data extraction selection for LLMM')
...
def parser_prompt_type(args):
    ...
    # ===== get the arguments 
        ...
        # Flag for a prompt to extract the information about the organization
    if args.info_Flag is not None:
        prmpt_settings.info_Flag = args.info_Flag
    else:
...
def parser_prompt_infi(args):
    # Create a prompt setting arguments ( if information is extracted ) that supports dot notation 
    prmpt_settings = DotDict()    
